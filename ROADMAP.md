...
- **ArangoDB architecture** — all artifact storage in ArangoDB; workspaces for ephemeral drafts, collections for committed, versioned records with graph relationships.
...
- **Search query language** — `+required`, `-excluded`, `tag:`, `type:`, `collection:`, `~semantic` operators; per-token semantic modifier. See [`docs/reference/search-query-language.md`](docs/reference/search-query-language.md).
...
- **11 tools** — `search`, `get_artifact`, `browse_collections`, `browse_workspaces`, `create_artifact`, `update_artifact`, `manage_artifact`, `extract_information`, `commit_preview`, `commit_workspace`, `ask`.
...
- [ ] **Inbound → Sage routing** — Nexus routes inbound message to Sage grounded-answer run.
...
- Ophan (finance/accounting) — all tools stub until platform primitives are stable
