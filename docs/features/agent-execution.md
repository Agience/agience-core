...
Each output artifact carries its own provenance in `context` — origin, timestamps,
actor, sources, evidence pointers, transform reference, and version lineage. The
inputs that drove the run, the tools that were invoked, and the linkage between
them are captured directly on the answer and evidence artifacts produced. Agience
does not emit a separate "run report" or receipt object — the artifacts themselves
*are* the audit trail.
