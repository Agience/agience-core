...
# STREAM_WEBHOOK_URI points at Mantle (Core infra), not a Chorus persona.
# Default targets Mantle inside the Docker network.
: "${STREAM_WEBHOOK_URI:=http://mantle:8081}"
...
export STREAM_WEBHOOK_PUBLISH="${STREAM_WEBHOOK_URI}${STREAM_WEBHOOK_PUBLISH_PATH}"
export STREAM_WEBHOOK_UNPUBLISH="${STREAM_WEBHOOK_URI}${STREAM_WEBHOOK_UNPUBLISH_PATH}"
