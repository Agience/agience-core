...
- run a thin local Agience host without the full Origin / Mantle backend (no Postgres, no ArangoDB)
...
- a source-tree MCP host app that mounts the persona servers directly from `src/chorus/`
...
- `AGIENCE_RELAY_ENABLED_PERSONAS=aria,sage,mantle,iris,astra,verso,seraph,ophan`
...
- confirm the selected persona exists under `src/chorus/`
