...
async def server_startup() -> None:
    """Run Aria startup tasks. With Phase C, the trust map is on disk; nothing to fetch."""
    await _auth.startup()


# ---------------------------------------------------------------------------
# Platform auth — Aria signs its own kernel JWT via the chorus service identity
# ---------------------------------------------------------------------------


async def _headers() -> dict[str, str]:
    """Return headers with Aria's own kernel JWT (for non-delegated calls)."""
    return _auth.headers()


async def _user_headers() -> dict[str, str]:
    """Return headers forwarding the caller's verified delegation JWT.

    When Mantle calls Aria on behalf of a user, it injects an RFC 8693 delegation
    token as the Authorization header. The middleware verifies and captures it
    into the per-request ContextVar; this function reads it for downstream REST
    callbacks so the user identity flows at the protocol level — never as a
    tool argument.

    Falls back to Aria's own kernel JWT when no delegation context is present
    (startup tasks, background work, direct server-to-server calls).
    """
...
async def extract_units(
    ...
    openai_key = config.LLM_API_KEY
    if not openai_key:
        return json.dumps({"error": "LLM_API_KEY not set on Aria server"})
...
async def attach_provenance(
    ...
    from kernel import config
    openai_key = config.LLM_API_KEY if mode_norm == "evidence" else None
...
async def _execute_external_tool(server_id: str, tool_name: str, arguments: dict) -> str:
    ...
    async with httpx.AsyncClient() as client:
        try:
            resp = await client.post(
                f"{AGIENCE_API_URI}/artifacts/{server_id}/op/invoke",
                headers=await _headers(),
...
async def _execute_chat_tool(name: str, arguments: dict, workspace_id: str | None) -> str:
    ...
    async with httpx.AsyncClient() as client:
        if name == "search":
            ...
            if not ws_id:
                return json.dumps({"error": "workspace_id required — list-all-workspaces moved to chorus/core's agent_browse_collection"})
            resp = await client.get(f"{AGIENCE_API_URI}/artifacts/{ws_id}/children", headers=user_headers, timeout=15)
            if resp.status_code < 400:
                items = resp.json()
            ...
            if not col_id:
                return json.dumps({"error": "collection_id required — list-all-collections moved to chorus/core's agent_browse_collection"})
            resp = await client.get(f"{AGIENCE_API_URI}/artifacts/{col_id}/children", headers=user_headers, timeout=15)
            if resp.status_code < 400:
                return json.dumps(resp.json())
            return json.dumps({"error": f"Failed: {resp.status_code}"})
...
async def _invoke_llm_via_verso(
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.post(
            f"{AGIENCE_API_URI}/artifacts/verso/op/invoke",
            headers=await _headers(),
            json={
                "name": "invoke_llm",
                "arguments": arguments,
...
async def _run_chat_turn_impl(
    ...
    openai_key = config.LLM_API_KEY
    if not openai_key:
        log.error("run_chat_turn: LLM_API_KEY not set and no connection_artifact_id provided")
        return json.dumps({"error": "No LLM connection available. Set LLM_API_KEY or provide a connection_artifact_id."})
