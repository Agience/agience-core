...
Stream
------
  Stream lifecycle (publish/unpublish) is now handled by Mantle core.
  The rotate_stream_key tool remains here to manage RTMP keys via the
  Core API.  SRS_HTTP_API is no longer needed on Astra.
...
async def _headers() -> dict[str, str]:
    """Headers with Astra's own kernel JWT (signed via the chorus service identity)."""
    return _auth.headers()


async def _user_headers() -> dict[str, str]:
    """Headers with the verified delegation JWT, or fall back to Astra's kernel JWT."""
...
async def server_startup() -> None:
    """Run Astra startup tasks. Trust map is on disk in Phase C; nothing to fetch."""
    await _auth.startup()
