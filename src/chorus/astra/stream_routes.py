...
Platform operations are performed via HTTP calls to Mantle. Phase C: each
call signs its own kernel JWT via the chorus service identity (no token
exchange, no PLATFORM_INTERNAL_SECRET).
...
# ---------------------------------------------------------------------------
# Platform auth — sign a chorus kernel JWT per request
# ---------------------------------------------------------------------------


def _sign_kernel_token() -> str:
    """Sign a fresh kernel JWT identifying Astra to Mantle.

    The chorus host's lifespan calls `init_service_identity("chorus")` before
    these routes are reachable.
    """
    return service_identity.sign_service_jwt(
        audience="mantle",
        additional_claims={"sub": STREAM_ROUTES_CLIENT_ID, "client_id": STREAM_ROUTES_CLIENT_ID},
    )


async def _headers(bearer: str | None = None) -> dict[str, str]:
    """Build request headers. Uses the provided bearer token if given;
    otherwise signs a fresh chorus kernel JWT for Astra."""
    h: dict[str, str] = {"Content-Type": "application/json"}
    token = bearer if bearer is not None else _sign_kernel_token()
    if token:
