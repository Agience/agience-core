...
async def _headers() -> dict[str, str]:
    """Headers with Ophan's own kernel JWT (signed via the chorus service identity)."""
    return _auth.headers()


async def _user_headers() -> dict[str, str]:
    """Headers with the verified delegation JWT, or fall back to Ophan's kernel JWT."""
...
def _profiles_root() -> Path:
    return _repo_root() / "package" / "profiles"
...
def _licensing_service():
    """Return the licensing module — lives next to this file in chorus/ophan/.
    ...
    sys.path yet (see `chorus/server.py`'s persona-load loop).
    """
...
# Licensing scope helpers — re-exported from kernel.scopes (the canonical
# definitions). The previous "inlined to avoid kernel import" comment was
# defensive; chorus has kernel on sys.path via the host's persona-load loop,
# and ophan/server.py's create_server_app is called inside that context.
from kernel.scopes import (
