"""Tests for chorus/ophan/licensing.py.

Was previously at mantle/tests/test_licensing_service.py when the licensing
module lived in mantle/services/. Moved alongside the module.
"""
import base64
import sys
...
# Put chorus/ophan/ on sys.path so `import licensing` resolves. The bare
# `licensing` name doesn't collide with anything else inmantle/  — this is
# the persona's own private module.
_HERE = Path(__file__).resolve().parent
sys.path.insert(0, str(_HERE.parent))

import licensing as licensing_service  # noqa: E402
