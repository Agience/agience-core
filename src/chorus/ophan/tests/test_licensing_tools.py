...
# Load ophan's server.py by file path so the bare `server` name doesn't
# collide with other personas' `server.py` in a bulk pytest run.
import importlib.util  # noqa: E402

_spec = importlib.util.spec_from_file_location(
    "chorus_ophan_server", _HERE.parent / "server.py"
)
ophan = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(ophan)
