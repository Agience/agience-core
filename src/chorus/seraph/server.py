...
--------------
  ...
  Note: Seraph's `_fetch_and_decrypt_secret` flow uses JWE secret delivery,
  which is deferred to Phase D (secrets-as-artifacts). Calls to that flow
  raise NotImplementedError until that lands.
...
async def _headers() -> dict[str, str]:
    """Headers with Seraph's own kernel JWT (signed via the chorus service identity)."""
    return _auth.headers()


async def _user_headers() -> dict[str, str]:
    """Headers with the verified delegation JWT, or fall back to Seraph's kernel JWT."""
