...
async def _headers() -> dict[str, str]:
    """Headers with Verso's own kernel JWT (signed via the chorus service identity)."""
    return _auth.headers()


async def _user_headers() -> dict[str, str]:
    """Headers with the verified delegation JWT, or fall back to Verso's kernel JWT."""
...
async def _dispatch_child_transform(
    ...
    resp = await client.post(
        f"{AGIENCE_API_URI}/artifacts/{transform_id}/op/invoke",
        headers=await _headers(),
        json=body,
...
async def execute_transform(
    ...
    if run_type == "mcp-tool":
        ...
        async with httpx.AsyncClient() as client:
            try:
                resp = await client.post(
                    f"{AGIENCE_API_URI}/artifacts/{server}/op/invoke",
                    headers=await _headers(),
                    json={
                        "name": tool,
                        "arguments": tool_args,
...
async def transcribe_artifact(
    ...
    if credential_artifact_id:
        ...
        # invoke path (POST /artifacts/seraph/op/invoke).
        try:
            async with httpx.AsyncClient(timeout=15) as client:
                seraph_resp = await client.post(
                    f"{AGIENCE_API_URI}/artifacts/seraph/op/invoke",
                    headers=await _headers(),
                    json={
...
async def invoke_llm(
    ...
    async with httpx.AsyncClient() as client:
        ...
        if resolution == "platform_default":
            ...
            api_key = config.LLM_API_KEY if provider == config.LLM_PROVIDER else None
        else:
            ...
            # Fallback to platform default if Seraph resolution didn't produce a key
            if not api_key:
                from kernel import config
                api_key = config.LLM_API_KEY if provider == config.LLM_PROVIDER else None
        ...
        try:
            ophan_check = await client.post(
                f"{AGIENCE_API_URI}/artifacts/ophan/op/invoke",
                headers=await _headers(),
                json={
...
@mcp.resource("ui://verso/vnd.agience.prompt.html")
async def prompt_viewer_html() -> str:
    """Serve the viewer HTML for vnd.agience.prompt+json.

    The platform adopts Verso's richer overlay for the prompt type: FACET's
    content-types build auto-derives `resource_uri: ui://verso/vnd.agience.prompt.html`
...
async def server_startup() -> None:
    """Run Verso startup tasks. Trust map is on disk in Phase C; nothing to fetch."""
    await _auth.startup()
