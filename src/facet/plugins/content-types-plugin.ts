...
function safeReadJson<T>(filePath: string): T | null {
  try {
    ...
    // Strip a leading UTF-8 BOM before parsing — parity with MANTLE's
    // `_read_json_file` (`encoding="utf-8-sig"`). Without this a BOM-prefixed
    // type.json (a common Windows-editor artifact) parses fine server-side but
    // is silently dropped here, leaving the type with behavior but no viewer.
...
function scanTypesRoot(
  ...
  // Detect server name from path: .../chorus/<name>/ui → "<name>"
  const serverMatch = typesRoot.replace(/\\/g, '/').match(/\/chorus\/([^/]+)\/ui$/);
  ...
  for (const top of topEntries) {
    ...
    for (const sub of subEntries) {
      ...
      const hasView = existsSync(join(subDir, 'view.html'));

      // Process a folder that declares a type/frontend manifest OR ships a
      // server-owned view.html. A view.html alone suffices to register a viewer
      // overlay: the content type comes from the folder path and the McpAppHost
      // resource_uri is auto-derived below — no redundant marker type.json needed
      // (the canonical type definition lives in package/types/).
      if (!typeJson && !frontendJson && !hasView) continue;
...
/**
 * Recursive merge matching MANTLE's `_deep_merge` (types_service.py): plain
 * objects recurse with the child winning per-key; everything else (scalars,
 * arrays, null) is replaced by the child. Kept byte-for-byte semantically equal
 * to the server so a nested override (e.g. one field of an inherited `ui` block)
 * merges rather than replacing the whole block — the two resolvers must agree.
 *
 * Exported for the conformance unit test.
 */
...
/** Merge inheritance: parent fields first, then own fields override (deep). */
function resolveOne(
  contentType: string,
  ...
  for (const parentMime of own.inherits ?? []) {
    ...
    merged = deepMerge(merged, parentMerged) as Record<string, unknown>;
  }

  // Own fields override parents, deep-merged (skip `inherits`; skip undefined,
  // but keep null — e.g. `viewer: null` is an intentional override).
  ...
  merged = deepMerge(merged, ownFields) as Record<string, unknown>;
  return merged as Partial<ScannedContentType>;
}
