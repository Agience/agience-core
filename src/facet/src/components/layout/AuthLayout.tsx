...
const AuthLayout: React.FC<AuthLayoutProps> = ({ children, hideLogo, cardClassName, footer }) => {
  ...
  return (
    <div key={mountKey} className="relative min-h-screen overflow-y-auto bg-gray-900">
      <style>{animationData.keyframes}</style>

      {/* Fixed background — stays in place even when card overflows viewport */}
      ...
      {/* Pause/play toggle — fixed so it stays in the corner when scrolling */}
      <button
        onClick={toggleEffects}
        className="fixed bottom-4 right-4 z-20 text-white/30 hover:text-white/60 transition-colors text-xs flex items-center gap-1"
      ...
      {/* Scrollable card wrapper — py-8 ensures card never touches viewport top/bottom */}
      <div className="relative z-10 flex min-h-screen items-center justify-center py-8 px-4">
        ...
        <div className={`w-full max-w-md rounded-2xl border border-white/30 shadow-[0_35px_60px_-15px_rgba(0,0,0,0.6)] ${cardClassName || ''}`}>
