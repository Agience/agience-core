...
const SetupWizard: React.FC = () => {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      if (emailProvider) {
        ...
        // Sentinel keys (_password_from_env, _api_key_from_env) are excluded — secrets injected by backend.
        for (const [k, v] of Object.entries(emailConfig)) {
          if (v && k !== 'from_address' && k !== 'from_name' && k !== '_password_from_env' && k !== '_api_key_from_env') {
    ...
    getSetupStatus()
      .then(status => {
        ...
        if (emailProviderEnv && ['smtp', 'ses', 'sendgrid', 'resend'].includes(emailProviderEnv)) {
          setEmailProvider(prev => prev || emailProviderEnv)
          const cfg: Record<string, string> = {}
          if (emailProviderEnv === 'smtp') {
            if (status.env_defaults?.smtp_host) cfg.host = status.env_defaults.smtp_host as string
            if (status.env_defaults?.smtp_port) cfg.port = status.env_defaults.smtp_port as string
            if (status.env_defaults?.smtp_username) cfg.username = status.env_defaults.smtp_username as string
            if (status.env_defaults?.smtp_has_password) cfg._password_from_env = 'true'
            if (status.env_defaults?.smtp_from) cfg.from_address = status.env_defaults.smtp_from as string
          } else if (emailProviderEnv === 'resend' && status.env_defaults?.resend_has_api_key) {
            cfg._api_key_from_env = 'true'
            if (status.env_defaults?.smtp_from) cfg.from_address = status.env_defaults.smtp_from as string
          } else if (emailProviderEnv === 'sendgrid' && status.env_defaults?.sendgrid_has_api_key) {
            cfg._api_key_from_env = 'true'
            if (status.env_defaults?.smtp_from) cfg.from_address = status.env_defaults.smtp_from as string
          }
          if (Object.keys(cfg).length > 0) {
            setEmailConfig(prev => Object.keys(prev).length > 0 ? prev : cfg)
            setEmailFromEnv(true)
          }
        }
      })
        ...
        {emailProvider === 'smtp' && (
          <div className="space-y-3 pt-2">
            ...
            <div className="space-y-1.5">
              ...
              {emailConfig._password_from_env === 'true' ? (
                <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-2 text-xs text-green-700">
                  ✓ Password configured from environment
                </div>
              ) : (
        ...
        {(emailProvider === 'sendgrid' || emailProvider === 'resend') && (
          <div className="space-y-3 pt-2">
            <div className="space-y-1.5">
              ...
              {emailConfig._api_key_from_env === 'true' ? (
                <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-2 text-xs text-green-700">
      ...
      <div className="space-y-3 text-sm">
        ...
        <div className="flex justify-between py-2 border-b border-gray-100">
          ...
          <span className="text-gray-900 font-medium">{
            emailProvider
              ? (emailFromEnv ? `${emailProvider.toUpperCase()} (from environment)` : emailProvider.toUpperCase())
              : 'Not configured'
          }</span>
        </div>
        <div className="flex justify-between py-2 border-b border-gray-100">
  ...
  return (
    <AuthLayout>
      ...
      {/* Content area: keeps a 400px floor for layout stability on short steps,
          but grows to fit taller steps (e.g. AI Access provider list) so options
          aren't clipped. Only scrolls internally when content would exceed the
          viewport; the action button stays pinned at the bottom. */}
      <div className="flex flex-col min-h-[400px] max-h-[70dvh]">
