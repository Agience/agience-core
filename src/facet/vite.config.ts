...
function discoverContentTypeRoots() {
  ...
  const roots = [resolve(repoRoot, 'package/types')]
  const chorusRoot = resolve(srcRoot, 'chorus')

  for (const entry of readdirSync(chorusRoot, { withFileTypes: true })) {
    if (!entry.isDirectory()) continue
    roots.push(resolve(chorusRoot, entry.name, 'ui'))
  }
