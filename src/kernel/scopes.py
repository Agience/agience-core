...
if TYPE_CHECKING:
    # Type-only — `core/` is shared between Origin and Mantle. Each service has
    # its own APIKey representation (Origin: origin/models/api_key.py,
    # Mantle: backend/entities/api_key.py). Both expose the same attribute
    # surface used by `check_scope` (scope_type, scope_subtype, etc.).
    from entities.api_key import APIKey as APIKeyEntity  # noqa: F401
