...
def get_arango_db() -> Generator[StandardDatabase, None, None]:
    ...
    Was previously at `kernel/dependencies.py`, but the implementation
    imports mantle's `schemas.arango.initialize`, so its kernel placement
...
def _validate_aud_for_principal(payload: dict) -> None:
    ...
    if principal_type == "service":
        # Phase C kernel mutual JWT: peer services (origin, chorus) calling
...
def resolve_auth(
    ...
    if payload and "sub" in payload:
        ...
        if jwt_principal_type == "service":
            # Phase C kernel mutual JWT — origin/chorus identifying themselves
