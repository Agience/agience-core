...
def _load_manifest() -> List[ManifestEntry]:
    """Load and validate ``chorus/manifest.json``."""
    repo_root = Path(__file__).resolve().parents[3]   # services/ → →→mantle/  → repo
    candidates = [
        repo_root / "src" / "chorus" / "manifest.json",   # local dev / source builds
    ...
    for p in candidates:
        ...
        raise FileNotFoundError(
            f"chorus/manifest.json not found. Searched: {[str(c) for c in candidates]}"
        )
