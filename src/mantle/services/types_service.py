...
def _default_server_ui_roots() -> List[Path]:
    """Walks first-party persona dirs for `ui/` subfolders."""
    chorus_root = _repo_root() / "src" / "chorus"
    if not chorus_root.exists() or not chorus_root.is_dir():
        return []

    roots: List[Path] = []
    for server_dir in sorted(chorus_root.iterdir(), key=lambda path: path.name.lower()):
...
def resolve_type_definition_cached(content_type: str) -> Optional[TypeResolutionResult]:
    ...
    Resolution order:
      ...
      2. Filesystem roots (``types/`` and ``chorus/*/ui/`` when present).
