...
from kernel import event_bus  # noqa: E402
...
def test_transform_type_declares_invoke_operation():
    """The transform type must declare operations.invoke dispatching to
    verso:execute_transform (the platform map — literal values in type.json)."""
    types_service.invalidate_type_cache()
    ...
    assert op.dispatch["tool_ref"] == "execute_transform"
...
async def test_dispatch_resolves_mcp_tool_refs_from_transform_artifact(monkeypatch):
    """Dispatching invoke on a transform artifact must call verso:execute_transform.
    The server_ref/tool_ref in type.json are literals — the platform map — so
    McpToolHandler always calls verso regardless of what is in artifact context.
    Post-Step-F: dispatch flows through chorus_client (HTTP→chorus gateway),
    not in-process mcp_service."""
    types_service.invalidate_type_cache()
    handler_registry.clear()
    ...
    def fake_call_tool(server_artifact_id, tool_name, arguments, *, user_id):
    ...
    monkeypatch.setattr(chorus_client, "call_tool", fake_call_tool)
    # Server registry needs ids populated for short-name resolution; stub it
    # to map "verso" → a deterministic UUID for assertion purposes.
    _name_to_uuid = {"verso": "11111111-1111-1111-1111-111111111111"}
    monkeypatch.setattr(
        server_registry,
        "resolve_name_to_id",
        lambda name: _name_to_uuid.get(name),
    )
    ...
    artifact = {
        ...
        "context": {
            ...
            "run": {"type": "mcp-tool", "server": "astra", "tool": "ingest_text"},
        },
    }
    ctx = DispatchContext(
        user_id="u1",
    ...
    # type.json declares verso:execute_transform as literals — the platform map.
    # McpToolHandler resolves "verso" → UUID via server_registry, then calls
    # chorus's universal gateway with the UUID-form id.
    assert captured["server"] == "11111111-1111-1111-1111-111111111111"
    assert captured["tool"] == "execute_transform"
...
def test_manifest_matches_servers_content_service():
    ...
    Count is 7: aria, astra, sage, iris, ophan, seraph, verso.
    (mantle and core personas removed; nexus renamed to iris.)
    """
    from services import server_registry
    assert len(server_registry.all_names()) == 7


def test_servers_content_service_context_shape():
    """The seeded server artifact context must carry the fields chorus's gateway
    looks up: content_type (dispatcher type resolution), transport (builtin
    routing), client_id (delegation JWT audience), name."""
    import yaml
    from pathlib import Path

    seed = (Path(__file__).resolve().parents[3] / "package" / "seeds" / "platform"
            / "artifacts" / "server-iris.yaml")
    ctx = yaml.safe_load(seed.read_text(encoding="utf-8"))["context"]
    assert ctx["content_type"] == "application/vnd.agience.mcp-server+json"
    assert ctx["mcp_server"]["transport"] == "builtin"
    assert ctx["mcp_server"]["client_id"] == "agience-server-iris"
    assert ctx["mcp_server"]["name"] == "iris"
    assert ctx["mcp_server"]["kind"] == "platform-builtin"
...
async def test_mcp_tool_handler_resolves_server_from_key_and_tool_from_body():
    """End-to-end check of the dispatch shape: given a server artifact with
    a `root_id`, and a body containing `{name, arguments}`, the mcp_tool
    handler resolves server_ref=$.root_id and tool_ref=$.body.name and
    invokes chorus_client.call_tool with the resolved values.
    Post-Step-F: chorus_client replaces in-process mcp_service.invoke_tool."""
    types_service.invalidate_type_cache()
    handler_registry.clear()
    handler_registry.register_builtin_handlers()
    ...
    monkeypatch_ctx.setattr(chorus_client, "call_tool", fake_call_tool)
    try:
        # The server artifact's root_id is itself a UUID — no server_registry
        # name lookup needed (the handler short-circuits on UUID-shaped ids).
        server_uuid = "22222222-2222-2222-2222-222222222222"
        artifact = {
            "_key": server_uuid,
            "root_id": server_uuid,
            "workspace_id": None,
        ...
        assert captured["server"] == server_uuid
...
def test_authorizer_type_declares_invoke_via_provide_access_token():
    """The authorizer type must declare operations.invoke dispatching to
    seraph:provide_access_token — without it, POST /artifacts/{id}/op/invoke
    (used by Iris send_email) raises OperationNotDeclared and email is dead."""
    types_service.invalidate_type_cache()
    op = types_service.resolve_operation(
        "application/vnd.agience.authorizer+json", "invoke"
    )
    assert op is not None
    assert op.requires_grant == "invoke"
    assert op.dispatch["kind"] == "mcp_tool"
    assert op.dispatch["server_ref"] == "seraph"
    assert op.dispatch["tool_ref"] == "provide_access_token"
    assert op.dispatch["arguments"] == {
        "authorizer_config": "$.content",
        "authorizer_artifact_id": "$.root_id",
    }


@pytest.mark.asyncio
async def test_authorizer_invoke_passes_artifact_content_and_id_to_seraph(monkeypatch):
    """End-to-end: invoking an authorizer artifact resolves the dispatch.arguments
    refs ($.content, $.root_id) against the artifact and calls
    seraph:provide_access_token with the resolved values."""
    types_service.invalidate_type_cache()
    handler_registry.clear()
    handler_registry.register_builtin_handlers()
    ...
    seraph_uuid = "33333333-3333-3333-3333-333333333333"
    monkeypatch.setattr(chorus_client, "call_tool", fake_call_tool)
    monkeypatch.setattr(
        server_registry,
        "resolve_name_to_id",
        lambda name: seraph_uuid if name == "seraph" else None,
    )

    authorizer_content = (
        '{"client_id": "cid", "client_secret_id": "sid", '
    ...
    artifact = {
        ...
        "root_id": "auth-1",
        "workspace_id": None,
        "content": authorizer_content,
        "context": {"content_type": "application/vnd.agience.authorizer+json"},
    }
    ctx = DispatchContext(
        user_id="u1", actor_id="u1", grants=[_FakeGrant(invoke=True)], arango_db=None
    )

    result = await dispatch("invoke", artifact, {}, ctx)
    assert result == {"access_token": "ya29.test"}
    assert captured["server"] == seraph_uuid
    assert captured["tool"] == "provide_access_token"
    assert captured["arguments"] == {
        "authorizer_config": authorizer_content,
        "authorizer_artifact_id": "auth-1",
    }
...
def test_dispatch_resources_read_extracts_uri_and_server_id_from_artifact(monkeypatch):
    from services import chorus_client, mcp_resource_importer

    captured = {}

    def fake_read_resource(server_artifact_id, uri, *, user_id):
        captured["server"] = server_artifact_id
        captured["uri"] = uri
        captured["user_id"] = user_id
        return {"contents": [{"uri": uri, "text": "hello"}]}

    monkeypatch.setattr(chorus_client, "read_resource", fake_read_resource)
    ...
    result = mcp_resource_importer.dispatch_resources_read(
        artifact,
        {"uri": "ui://aria/chat", "workspace_id": "ws-1"},
        ctx,
    )
    assert result == {"uri": "ui://aria/chat", "text": "hello"}
...
def test_dispatch_resources_read_raises_on_missing_uri():
    from services import mcp_resource_importer
    artifact = {"_key": "srv-1", "context": {}}
    ctx = DispatchContext(user_id="u1", actor_id="u1", grants=[], arango_db=None)
    with pytest.raises(ValueError, match="body.uri"):
        mcp_resource_importer.dispatch_resources_read(artifact, {}, ctx)
...
def test_dispatch_resources_import_wraps_existing_service(monkeypatch):
    ...
    monkeypatch.setattr(mcp_resource_importer, "import_resources_as_artifacts", fake_import)
    ...
    ctx = DispatchContext(user_id="u1", actor_id="u1", grants=[], arango_db=None)
    result = mcp_resource_importer.dispatch_resources_import(
        artifact,
...
def test_dispatch_resources_import_raises_on_missing_workspace():
    ...
    with pytest.raises(ValueError, match="workspace_id"):
        mcp_resource_importer.dispatch_resources_import(artifact, {"resources": []}, ctx)
...
async def test_end_to_end_resources_read_via_dispatcher(monkeypatch):
    ...
    by dotted name (`mcp_resource_importer.dispatch_resources_read`),
    importlib fallback imports `services.mcp_resource_importer`, dispatcher
    invokes it with `(artifact, body, ctx)`, result comes back through the
    emit envelope with mcp.resource.read event fired."""
    types_service.invalidate_type_cache()
    handler_registry.clear()
    handler_registry.register_builtin_handlers()
    ...
    # dispatch_resources_read flattens the MCP ReadResourceResult to the first
    # content item — the flat MCPResourceContents shape the frontend expects.
    assert result == {"uri": "ui://aria/chat", "text": "from-aria"}
