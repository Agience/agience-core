...
from services.dependencies import get_arango_db  # noqa: E402
...
class TestArtifactLookup:
    ...
    def test_check_access_uses_doc_key_not_root_id(self, app, mock_db):
        ...
        iris_root_id = "aaaabbbb-cccc-dddd-eeee-ffffddddeeee"
        version_key = "iris-version-key-202604"
        version_doc = {
            "_key": version_key,
            "id": version_key,
            "root_id": iris_root_id,
            "context": {
                "content_type": "application/vnd.agience.mcp-server+json",
                "mcp_server": {"name": "iris", "transport": "builtin"},
            },
        }
        ...
        with (
            ...
            resp = client.post(
                f"/artifacts/{iris_root_id}/op/resources_read",
                json={"uri": "ui://iris/vnd.agience.mcp-server.html", "workspace_id": "ws-live"},
            )
        ...
        assert checked_with[0] == version_key, (
            ...
            f"using root_id '{iris_root_id}' causes a 404 inside check_access → 403 from dispatcher"
        )
    ...
    def test_uuid_passthrough_unchanged(self, authed_client, mock_db):
        ...
        with patch("services.operation_dispatcher.dispatch", new=AsyncMock(return_value={"result": "ok"})):
            resp = authed_client.post(
                ...
                json={"uri": "ui://iris/vnd.agience.mcp-server.html"},
            )
...
class TestReservedOpNames:
    ...
    @pytest.mark.parametrize("op_name", ["create", "read", "update", "delete", "add", "search"])
    def test_reserved_op_names_return_400(self, op_name, authed_client, mock_db):
        """Reserved CRUD names belong to dedicated routes; op/{op_name} rejects them.

        Note: `invoke` was previously reserved but is now accepted by op/{op_name}
        with a body-merge pass (see _build_invoke_dispatch_body). The dedicated
        /invoke route was deleted and callers moved to /op/invoke.
        """
        resp = authed_client.post(
...
class TestDispatchHappyPath:
    def test_op_dispatch_returns_dispatcher_result(self, authed_client, mock_db):
        ...
        expected_result = {"uri": "ui://iris/view.html", "text": "<html>viewer</html>"}
        with patch("services.operation_dispatcher.dispatch", new=AsyncMock(return_value=expected_result)):
            resp = authed_client.post(
                ...
                json={"uri": "ui://iris/view.html", "workspace_id": "ws-1"},
            )
